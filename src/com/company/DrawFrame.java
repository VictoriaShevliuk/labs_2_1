...
public class DrawFrame extends JFrame {
    ...
    private ArrayList<Float[]> outerPolyPoints;

    public DrawFrame(String name, ArrayList<Float[]> basePolyPoints, ArrayList<Float[]> points){
        this(name, basePolyPoints, points, null);
    }

    public DrawFrame(String name, ArrayList<Float[]> basePolyPoints, ArrayList<Float[]> points, ArrayList<Float[]> outerPolyPoints){
        super(name);

        this.outerPolyPoints = outerPolyPoints;
    ...
    private void initFrame(){
        ...
        Graphic graphic = new Graphic(basePolyPoints, points, outerPolyPoints, getSize());
...
class Graphic extends JComponent {
    ...
    public Graphic(ArrayList<Float[]> basePolyPoints, ArrayList<Float[]> points, ArrayList<Float[]> outerPolyPoints, Dimension parentSize){
        this.basePolyPoints = basePolyPoints;
        this.points = points;
        this.outerPolyPoints = outerPolyPoints;
    ...
    public void paint(Graphics g) {
        ...
        if(outerPolyPoints != null){
            xPoints = new int[outerPolyPoints.size()];
            yPoints = new int[outerPolyPoints.size()];

            for (int i = 0; i < outerPolyPoints.size(); i++){
                xPoints[i] = outerPolyPoints.get(i)[0].intValue();
                yPoints[i] = outerPolyPoints.get(i)[1].intValue();
            }

            g2D.setColor(outerPolyColor);
            Polygon outerPolygon = new Polygon(xPoints, yPoints, outerPolyPoints.size());
            g2D.drawPolygon(outerPolygon);
        }
    }
